$(function(){
    ...
    $("#fullpage").imagesLoaded().done(function(){ //프리로딩 완료 이벤트
        ...
        setTimeout(function(){
            $(".wrap-loading").remove(); //0.7초 후 프리로딩 마크업을 삭제처리
        },700)
        ...
        $(".wrap-loading .num").text(_percent).css("width", _percent + "%"); //프리로딩 퍼센트 숫자 표시 마크업에 퍼센트
    });
    ...
    $("#fullpage").fullpage({
        ...
        onLeave:function(name, index, direction){
            if(index == 1){//index가 1일 경우}
              $("#second0 .ico").css("transform","translateY(0)")
        }else{//index가 1이 아닐 경우
              $("#second0 .ico").css("transform","translateY(-300px)")
        };
     },
